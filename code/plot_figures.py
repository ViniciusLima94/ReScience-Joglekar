...
def fig1b(t, Rweak, Rstrong):
	plt.plot(t/1000, Rweak, 'g', lw = 3, label='weak LBA')
	plt.plot(t/1000, Rstrong, 'purple', lw = 3, label='strong LBA')
	plt.xlim([0,0.6])
	plt.ylim([0,6.0])
	plt.xlabel('Time (s)')
	plt.ylabel('Excitatory rate (Hz)')
	plt.legend()
	plt.savefig('figures/fig1_b.pdf', dpi = 600)
	plt.close()

def fig1c(Fmax, extent):
	...
	plt.xlabel('Local E to E coupling')

	Wee = 6.00; Wei = 6.70
	plt.plot(Wee, Wei, 'purple', marker='o',ms=10)
	Wee = 4.45; Wei = 4.70
	plt.plot(Wee, Wei, 'go', ms=10)

	plt.savefig('figures/fig1_c.pdf', dpi = 600)
	plt.close()
...
def fig3_f(muee_vec, max_r_24c_t, max_r_24c_f):
	...
	plt.xlabel('Global E to E coupling')
	plt.ylabel('Maximum rate in 24c [Hz]')
...
def plot_raster(times_ex, index_ex, reg, simtime, filename, save=False):
	...
	plt.plot(times_ex[ind_ex], index_ex[ind_ex], 'b.', markersize=.7, label='exc')
