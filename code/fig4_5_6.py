...
def simulate(
    ...
    (
        ...
        I_ext,
        inputs,
    ) = setParams.get_params_spiking_model(dt, reg=reg, gba=gba)
    ...
    if use_default_noise == True:
        nest.SetStatus(
            xi_e, [{"mean": I_ext[reg][gba]["Ie"], "std": std_ex, "dt": 0.1}]
        )
        nest.SetStatus(
            xi_i, [{"mean": I_ext[reg][gba]["Ii"], "std": std_in, "dt": 0.1}]
        )

    else:
        nest.SetStatus(xi_e, [{"mean": Ie_mean, "std": std_ex, "dt": 0.1}])
        nest.SetStatus(xi_i, [{"mean": Ii_mean, "std": std_in, "dt": 0.1}])
            ...
            [
                {
                    "amplitude_times": [
                        inputs[reg][gba]["t_on"],
                        inputs[reg][gba]["t_off"],
                    ],
                    "amplitude_values": [inputs[reg][gba]["I"], 0.0],
                }
            ],
        )
    ...
    for pop in range(Nareas):
        ...
        nest.Connect(
            ...
            pop_e[pop],
            conn_exc,
            {"delay": d, "weight": (1.0 + eta * netwParams_hier[pop][0]) * wee},
        )
        nest.Connect(
            pop_e[pop],
            pop_i[pop],
            conn_exc,
            {"delay": d, "weight": (1.0 + eta * netwParams_hier[pop][0]) * wie},
        )

        # Create inhibitory connections
        nest.Connect(pop_i[pop], pop_e[pop],
                     conn_inh, {"delay": d, "weight": -wei})
        nest.Connect(pop_i[pop], pop_i[pop],
                     conn_inh, {"delay": d, "weight": -wii})
    ...
    for i, j in zip(pre, pos):
        nest.Connect(
            ...
            conn_exc,
            {
                "delay": nest.random.normal(
                    mean=delayMat[j, i], std=delayMat[j, i] * 0.1
                ),
                "weight": W_ee_mat[j, i],
            },
        )
    ...
    rate_ex = (
        np.sum(nest.GetStatus(spikes_e)[0]["events"]["times"] >= transient)
        / (simtime - transient)
        / (NE * Nareas)
        * 1e3
    )
    rate_in = (
        np.sum(nest.GetStatus(spikes_i)[0]["events"]["times"] >= transient)
        / (simtime - transient)
        / (NI * Nareas)
        * 1e3
    )
