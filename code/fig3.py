...
def simulate(lnt=1, simtime=1000.0, dt=0.2, params=None, max_cond=True,
             seed=0):
    ##########################################################################
    ...
    for i in range(Nareas):
        ...
        nest.SetStatus(pop[i][0], {"tau": tau_ex, "g": 1.0,
                                   "rate": params["I"]["exc"]})
        # Inhibitory population
        nest.SetStatus(pop[i][1], {"tau": tau_in, "g": 1.0,
                                   "rate": params["I"]["inh"]})
        if max_cond:
    ...
    nest.SetStatus(multi, {"record_from": ["rate"], "interval": 0.2})
    for i in range(Nareas):
    ...
    Iext_e = nest.Create("threshold_lin_rate_ipn", 1)  
    Iext_i = nest.Create("threshold_lin_rate_ipn", 1)  
    ...
    nest.SetStatus(
        Iext,
        {
            "amplitude_times": [2000.0, 2250.0],
            "amplitude_values": [params["Iext"], 0.0],
        },
    )
    conn = {"rule": "one_to_one"}
    syn_e = {"weight": 1.0, "delay": 0.1,
    ...
    for n in range(Nareas):
        ...
        # I->E
        syn_ei = {"weight": local_ei,
                  "synapse_model": "rate_connection_instantaneous"}
        nest.Connect(pop[n][1], pop[n][0], conn, syn_ei)
        # I->I
        syn_ii = {"weight": local_ii,
                  "synapse_model": "rate_connection_instantaneous"}
