...
def simulate(lnt=1, simtime=600.0, dt=0.2, transient=0, tau=20.0, Wee=6.00,
             Wei=6.70):
    ##########################################################################
    ...
    nest.SetKernelStatus(
        {
            ...
            "local_num_threads": lnt,  # threads to build/simulate the network
            "rng_seed": lnt,
        }
    )
    nest.SetDefaults(
    ...
    return (
        times[int(transient / dt) :] - times[int(transient / dt) :].min(),
        rates[int(transient / dt) :],
    )
